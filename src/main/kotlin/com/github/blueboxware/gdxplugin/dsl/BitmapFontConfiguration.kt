...
data class OutputFontSpec(var fontSize: Int, var file: Any? = null)

abstract class BitmapFontConfiguration @Inject constructor(@Input var name: String, objectFactory: ObjectFactory) {

  @Nested
  val settings: Property<BitmapFontSettings> = objectFactory.property<BitmapFontSettings>().convention(
    BitmapFontSettings()
  )

  @Input
  val inputFont: Property<Any> = objectFactory.property<Any>().convention("Arial")

  @get:Internal
  abstract val outputFile: Property<Any>
  ...
  @get:Input
  @get:Optional
  abstract val defaultName: Property<String>

  @Internal
  val outputFonts: MutableList<OutputFontSpec> = mutableListOf()

  fun settings(closure: Closure<in BitmapFontSettings>) {
    settings.get().configure(closure)
  }

  fun settings(closure: BitmapFontSettings.() -> Unit) {
    settings.get().apply(closure)
  }
  ...
  fun size(size: Int, filename: String) =
    outputFonts.add(OutputFontSpec(size, filename))
  ...
  val EXTENDED = listOf(
    ...
    11382,
    11383
  ).map { it.toChar() }.joinTo(StringBuilder(), separator = "").toString()
